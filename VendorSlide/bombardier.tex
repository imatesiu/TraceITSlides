...
CITYFLO 650 can also be used as an \textbf{overlay} train control system or \textbf{resignalling} % to upgrade existing fixed block (brown field) systems.
\end{block}
...
\begin{itemize}
\item The RF communication uses either a \textbf{leaky coaxial cable or Line of Sight }(LOS) antenna network along the wayside that transmits data to the trains via their onboard mobile antennas.
\item The train has also an internal communication network, which uses the \textbf{IEEE-1473 protocol}.
\end{itemize}
\end{block}
...
\begin{itemize}
\item {\scriptsize CITYFLO 650} is capable of less than \textbf{75s headway}
\item  \textmd{Metro Madrid Line} 1\&6: \textbf{101s/111s headway}
...
No Information
   ...
   \end{block}
      ...
      \begin{itemize}
       \item Tracks divided into entity called a\textbf{ ``{\scriptsize CITYFLO Segment}''} .  
...
\item  The ATO system allows precision stopping by the train at stations with a typical \textbf{ accuracy of +/- 15 cm} 
\end{itemize}
